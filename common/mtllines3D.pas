unit mtllines3D; //OpenGL and Metal differ in only in first 2 lines
{$DEFINE METALAPI}
...
{$IFDEF METALAPI}
procedure TGPULines3D.SetPipeline();
var
...
procedure  TGPULines3D.CreateCube(sz: single);
{$IFNDEF METALAPI}
const
...
procedure TGPULines3D.SetAzimuth(f: single);
begin
  //if (f <> fAzimuth) then isRedraw := true;
  fAzimuth := f;
end;

procedure TGPULines3D.SetElevation(f: single);
begin
  //if (f <> fElevation) then isRedraw := true;
  fElevation := f;
end;

procedure TGPULines3D.SetIsText(f: boolean);
begin
     if (f <> isText) then isRedraw := true;
     isText := f;
end;

procedure TGPULines3D.SetIsTopLeft(f: boolean);
begin
     if (f <> isTopLeft) then isRedraw := true;
     isTopLeft := f;
end;

procedure  TGPULines3D.SetSize(f: single);
begin
...
procedure  TGPULines3D.ScreenSize(Width,Height: integer);
begin
...
{$IFDEF METALAPI}
constructor  TGPULines3D.Create(Ctx: TMetalControl);
{$ELSE}
constructor  TGPULines3D.Create(Ctx: TOpenGLControl);
{$ENDIF}
begin
     scrnH := 0;
     SizeFrac := 0.02;
...
procedure  TGPULines3D.Draw(Width,Height: integer);
var
  sz: single;
  {$IFDEF METALAPI}
