...
fragment float4 fragmentShader(VertexOut  in [[stage_in]],
	...
	while (lengthAcc <= len) {
		#ifdef GRADIENT
			colorSample.rgba = (gradTexture.sample(textureSampler, samplePos)).rgba;
		#else
			float tex = (volTexture.sample(textureSampler, samplePos)).r ;
			colorSample.rgba = float4(tex,tex,tex,tex );
		#endif
